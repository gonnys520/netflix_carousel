...
function extendSliderFunForVimeo() {
	this.postToIframe = function(a, d, b) {
		...
		if (window.JSON) {
			...
			if (b)
				c.value = b;
			this.iframe.contentWindow.postMessage(JSON.stringify(c), a
					.getAttribute("src").split("?")[0])
		}
	};
	...
	if (LIS_VMs.length) {
		...
		function b(d, a) {
			if (window.JSON) {
				...
				if (a)
					e.value = a;
				for (var b = 0; b < LIS_VMs.length; b++)
					c(d, a, LIS_VMs[b].player)
			}
		}
		function c(d, b, a) {
			var c = {
			...
			if (b)
				c.value = b;
			a.contentWindow.postMessage(JSON.stringify(c), a
					.getAttribute("src").split("?")[0])
		}
		function g() {
...
function addYoutubeEventHandlers(c) {
	if (LIS_YTs.length) {
		...
		for (var a = 0; a < LIS_YTs.length; a++)
			LIS_YTs[a].player.src += "&origin=" + location.protocol + "//"
					+ location.hostname
